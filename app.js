...
const css = `
  ...
  *{box-sizing:border-box;margin:0;padding:0}
  body{background:#0d0d12;color:#e8e2d9;font-family:system-ui,sans-serif;min-height:100vh}
  ...
  .logo{font-family:system-ui,sans-serif;font-weight:800;font-size:22px;color:#fff}
  ...
  .tab{background:none;border:none;border-bottom:2px solid transparent;color:#555;font-family:system-ui,sans-serif;font-weight:700;font-size:12px;letter-spacing:1px;padding:8px 0;cursor:pointer;transition:.2s}
  ...
  .balance{font-family:system-ui,sans-serif;font-weight:800;font-size:22px;letter-spacing:-0.5px;margin-top:6px;word-break:break-word;line-height:1.2}
  .grid2{display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:12px}
  .mini-amt{font-family:system-ui,sans-serif;font-weight:700;font-size:15px;margin-top:6px;word-break:break-word;line-height:1.3}
  ...
  .tx-amt{font-family:system-ui,sans-serif;font-weight:700;font-size:13px;flex-shrink:0;text-align:right}
  ...
  .type-btn{flex:1;padding:10px;border-radius:10px;border:1.5px solid;font-family:system-ui,sans-serif;font-weight:700;font-size:13px;cursor:pointer;transition:.2s}
  .chips{display:flex;flex-wrap:wrap;gap:8px;margin-bottom:18px}
  .chip{border-radius:6px;padding:6px 12px;font-size:11px;border:1.5px solid;cursor:pointer;font-family:system-ui,sans-serif;transition:.2s}
  .input{background:#0d0d12;border:1.5px solid #222230;border-radius:10px;color:#e8e2d9;font-family:system-ui,sans-serif;font-size:14px;padding:10px 14px;width:100%;outline:none;margin-bottom:12px;transition:.2s}
  .input:focus{border-color:#c8ff57}
  .add-btn{background:#c8ff57;color:#0d0d12;border:none;border-radius:10px;font-family:system-ui,sans-serif;font-weight:800;font-size:15px;padding:14px;width:100%;cursor:pointer;margin-top:8px;transition:.2s}
...
function App() {
  ...
  const addTx = () => {
    ...
    setErr("");
    setTxs(p=>[...p,{...form,amount:amt,id:Date.now()}]);
    setForm(f=>({...f,category:"",amount:"",note:"",date:now.toISOString().slice(0,10)}));
    setView("log");
  };
    ...
    React.createElement("div", {className:"wrap"},
      React.createElement("div", {className:"header"},
        React.createElement("div", null,
          React.createElement("div", {className:"logo"}, "ledger", React.createElement("span", null, ".")),
          React.createElement("div", {className:"logo-sub"}, "PERSONAL FINANCE")
        ),
        React.createElement("div", {className:"month-nav"},
          React.createElement("button", {onClick:prevM}, "‹"),
          React.createElement("span", {className:"month-label"}, MONTHS[month]+" "+year),
          React.createElement("button", {onClick:nextM}, "›")
        )
      ),
      React.createElement("div", {className:"tabs"},
        [["dashboard","Overview"],["log","Transactions"],["add","+ Add"]].map(([v,l])=>
          React.createElement("button", {key:v, className:"tab"+(view===v?" active":""), onClick:()=>setView(v)}, l)
        )
      ),
      ...
      view==="dashboard" && React.createElement(React.Fragment, null,
        ...
        breakdown.length>0 && React.createElement("div", {className:"card"},
          React.createElement("div", {className:"label", style:{marginBottom:14}}, "EXPENSE BREAKDOWN"),
          breakdown.map(([id,amt])=>{
            const m = getCat("expense",id);
      ...
      view==="log" && React.createElement(React.Fragment, null,
        ...
        filtered.map(t=>{
          const m = getCat(t.type,t.category);
          return React.createElement("div", {key:t.id, className:"card tx"},
            React.createElement("div", {className:"tx-icon", style:{background:t.type==="income"?"rgba(200,255,87,.12)":"rgba(255,107,107,.1)"}}, m.icon),
      ...
      view==="add" && React.createElement("div", {className:"card"},
        React.createElement("div", {style:{fontFamily:"'Syne',sans-serif",fontWeight:700,fontSize:16,marginBottom:18}}, "New Transaction"),
        React.createElement("div", {className:"type-row"},
        ...
        React.createElement("div", {className:"label", style:{marginBottom:10}}, "CATEGORY"),
        ...
        React.createElement("div", {className:"label", style:{marginBottom:8}}, "DATE"),
        React.createElement("input", {className:"input",type:"date",value:form.date,onChange:e=>setForm(f=>({...f,date:e.target.value}))}),
        err && React.createElement("div", {className:"err"}, "⚠ "+err),
        React.createElement("button", {className:"add-btn",onClick:addTx}, "Add Transaction")
      )
    )
  );
}
